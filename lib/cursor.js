...
// We aren't wrapping the `each` callback in a fiber, so any mongo calls in the callback will fail
Cursor.prototype.forEach = function(each, cb) {
  return syncOrAsync(this._cursor, 'forEach', [each], undefined, cb)
}
